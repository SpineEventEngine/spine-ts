...
| TM-005 | D      | Subscription memory/CPU and update delivery; TB-01/TB-02                            | Caller able to reach Subscribe creates valid inactive/active work or becomes a slow consumer in one service process.                                                            | SF-008 default-100 ID reservations cover pending, inactive, active, and recovery; queue/filter/TTL bounds and rollback/release tests are in `spine-services.ts` and `spine-services.test.ts`.                              | SF-008 implemented; re-review lifecycle release paths. Distributed/per-tenant quotas and rate limits remain deployment-owned.                                                                                                                                                                        | High before fix; residual Medium at scale         |
