...
findings. Canonical review and dedicated security re-review are pending.

Baseline: `39f2c6f7`. Immutable implementation and review endpoints are recorded
in the T-0041 task, work, and review logs.

Canonical wave 3 runtime review basis: `ffd8549e`, represented by
`.superpowers/sdd/review-39f2c6f7..ffd8549e.diff`. Later documentation-only
commits do not change that runtime basis. Canonical review and dedicated
...
| TM-005 | D      | Subscription memory/CPU and update delivery; TB-01/TB-02                            | Caller able to reach Subscribe creates valid inactive/active work or becomes a slow consumer in one `SpineServices` instance.                                                   | SF-008 default-100 per-instance ID reservations cover pending, inactive, active, and recovery; owner-scoped recovery cancellation and resident-removal tombstones plus queue/filter/TTL bounds are tested in `spine-services.test.ts`. | SF-008 implemented; canonical and security re-review pending. Each instance has an independent limit; distributed/per-tenant quotas and rate limits remain deployment-owned.                                                                                                                         | High before fix; residual Medium at scale         |
