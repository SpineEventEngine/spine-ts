...
- ZeroMQ is same-host IPC. It requires an absolute directory and checks mode
  0700 privacy (`adapter-config.ts:33-45`; `signal-transport.ts:592-602`).
...
| Tenant state/delivery    | Tenant records and service paths: `tenant-records.ts:12-71`; `spine-services.ts:247-309`. |
...
| TB-02 | RPC request -> bounded context          | Route/type/tenant validation (`spine-services.ts:187-230,247-274`).                                             |
...
| TB-06 | Framework -> same-host ZeroMQ peer      | Absolute private directory and timeouts (`adapter-config.ts:33-56`; `signal-transport.ts:42-47,168-196`).       |
...
| TM-004 | D      | HTTP/2 sessions, payload memory, runtime queue; TB-01/TB-02                         | Broadly exposed unauthenticated client holds sessions, sends large payloads, or drives accepted work faster than handlers settle.                      | Loopback default and ordered session cleanup (`server.ts:18-19,84-113`); runtime explicitly has no hostile-callback queue bound (`runtime.ts:47-58`).                                 | Hypothesis; inspect Connect/HTTP2 payload/session bounds. TLS, ingress payload/rate policy is deployment-owned.                       | High when exposed                   |
| TM-005 | D      | Subscription memory/CPU and update delivery; TB-01/TB-02                            | Caller creates many valid subscriptions, deep filters, or slow consumers that retain queued updates.                                                   | Queue, ID, filter, depth, mask, and path limits (`spine-services.ts:133-160,1206-1460`).                                                                                              | Hypothesis; verify per-subscription queue overflow, inactive TTL, removal, and slow-consumer behavior.                                | Medium; High at scale               |
...
| TM-007 | S/T/D  | Local IPC integrity, confidentiality, availability; TB-06                           | Same-host user can access/manipulate IPC path or send oversized serialized frames, then spoofs routing, races endpoints, or exhausts deserialize work. | Absolute path/identity validation, private-directory check, socket timeouts (`adapter-config.ts:33-56`; `signal-transport.ts:42-47,168-196,584-602`).                                 | Hypothesis; frame size and peer identity are not established controls; inspect endpoint hashing/cleanup and oversized-frame behavior. | Medium with local access            |
