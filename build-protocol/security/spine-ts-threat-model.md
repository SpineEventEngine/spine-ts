...
Committed pre-wave-4 review basis: `5a165d07`, represented by
`.superpowers/sdd/review-39f2c6f7..5a165d07.diff`. The Wave 4 working endpoint
has no commit hash; its immutable implementation/review endpoints belong in the
T-0041 task, work, and review logs once available. Canonical review and
...
| TM-005 | D      | Subscription memory/CPU and update delivery; TB-01/TB-02                            | Caller able to reach Subscribe or Cancel creates valid inactive/active work, many unknown-ID removals, or becomes a slow consumer in one `SpineServices` instance.              | SF-008 default-100 Set reservations cover pending, inactive, active, and recovery; unknown removals have a separate equal bound. Exact owner claims and cancel markers fence cross-instance recovery/cancel races; focused tests cover both CAS orderings, cleanup failure, saturation, queue, TTL, and attachment cleanup (`spine-services.ts`; `spine-services.test.ts`). | Wave 4 implemented in the working tree; canonical and security re-review pending. Each instance has independent limits. A crashed owner can leave a stale claim because no lease/reclamation contract exists; distributed/per-tenant quotas and rates remain deployment-owned.                       | High before fix; residual Medium at scale         |
