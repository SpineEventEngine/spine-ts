...
- Public examples use package imports only and keep end-user code free of
  framework envelopes, manual transactions, event replay, schema-bearing
...
- Legacy compatibility symbols retain narrow accepted wording; docs do not
  recommend new event replay, import, raw callback-delivery, or manual-transaction use.
...
- Reconciled claims: root/package ownership follows each package manifest and
  ...
  exclusions make no future-policy commitment; legacy `IMPORT_EVENT`, event replay,
