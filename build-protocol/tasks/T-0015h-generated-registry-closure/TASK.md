...
- Ordinary aggregate code must not use event replay, create framework event IDs, or
...
- Active example source scan found no committed example use of
  `defineEntityHandlers()`, `materializeDecoratedEntityHandlers()`, event replay,
...
- Cleanup enforcement already rejected schema-bearing decorators, framework
  envelopes, event IDs, manual materialization, event replay, and app-owned
