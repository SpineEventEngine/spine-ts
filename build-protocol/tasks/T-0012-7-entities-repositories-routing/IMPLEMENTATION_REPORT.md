...
Implemented the repository registration and storage-opening slice:

- `Repository` now retains entity/state metadata and can register with one
  built `BoundedContext`.
- Registration opens a `RecordStorage` through the context `StorageFactory`
  using the repository state schema.
- Repeated registration with the same built context is idempotent.
...
- `BoundedContext.registeredRepositories()` exposes a copy-safe registered
  repository list for later routing slices.
- README and user-guide sections now describe the real registration behavior
...
No independent reviewer sub-agents were spawned because the implementation
instruction explicitly said not to spawn agents. The implementation was
self-checked against the review exclusions and verified by the commands below.
...
- `registeredRepositories()` currently returns the repository instances as a
  copy-safe array, not a read-only wrapper around each repository. This is
  intentional for later routing, and follows the task allowance for a small
  registered repository view.
- Direct `Repository.registerWith(context)` is available for lifecycle parity
  with the JVM model. Context-owned registration lists are populated by
