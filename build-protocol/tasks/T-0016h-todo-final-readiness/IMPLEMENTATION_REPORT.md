...
- Updated the to-do README and user guide so build, start, and focused-test
  commands are copy-pasteable with
...
- `pnpm --config.verify-deps-before-run=false vitest run examples/todo/src/index.test.ts --passWithNoTests`:
  failed in the managed sandbox with `listen EPERM: operation not permitted
  127.0.0.1`; reran with native loopback approval and passed with 1 file and
  19 tests.
