...
- Keep end-user code free of framework `Event` envelopes, manual transactions,
  event replay, schema-bearing decorators, and application-owned handler
  materialization.
...
- Event import, `ImportBus`, aggregate importers, aggregate event replay, or new
  `IMPORT_EVENT` writes.
...
- No application code sees framework `Event` envelopes, manual transactions,
  event replay, schema-bearing decorators, or materialization helpers.
