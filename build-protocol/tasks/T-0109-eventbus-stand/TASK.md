...
| Use the application `StorageFactory` by default and allow a custom registry at the Bounded Context builder. | The default durable registry is created from context storage, while the builder accepts a complete user-supplied registry implementation.                                      |
| Clean abandoned pending activations after a finite 30-second interval.                                      | Each application node runs the same idempotent pending-activation cleanup against the shared registry.                                                                         |
| Delete cancelled subscriptions physically rather than retaining tombstones.                                 | Cancellation removes the durable definition; reconciliation detaches local consumers after observing its absence.                                                              |
| Warn, but do not fail, when production retains an in-memory Stand registry.                                 | Production startup emits a WARN-level diagnostic and continues so applications without subscriptions remain fully functional.                                                  |
