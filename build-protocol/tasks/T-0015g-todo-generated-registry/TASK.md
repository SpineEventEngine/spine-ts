...
envelopes from command handlers, create framework event IDs, use event replay, or
...
- event replay is not supported for new aggregate code; aggregates are moving away
  from event sourcing.
...
- Adding event replay compatibility.
...
- To-do command handlers return generated domain event messages, not framework
  `Event`.
- To-do aggregate code contains no event replay, app-managed transaction calls, or
