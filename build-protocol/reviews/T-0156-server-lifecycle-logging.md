...
Coverage correction: prior changed-range readiness was premature (branch coverage was 84.44%). New tests cover foreign logger access/clear behavior; fresh serial server coverage passes 69 files / 1,645 tests and its exact LCOV is at `/tmp/t0156-coverage/lcov.info` for recalculation before any reviewer dispatch. Style and performance/reliability remain pending; do not treat the task as mechanically pre-review-ready until every exact changed-range metric is confirmed >=90.
