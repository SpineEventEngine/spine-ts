...
| `P-02` | **Partly true**      | Partial feature           | `BoundedContext.catchUpReadSide()` can rebuild every event-subscribing Projection in one context and tenant by clearing state and replaying the full stored event log directly. It cannot select one Projection type, IDs, or a starting time, and it has no durable job, checkpoint, Inbox `CATCH_UP` delivery, cross-process live-event coordination, or resumption. Preserve the useful local API, then add JVM-grade selective/durable capability as a later feature. |
...
### Wave 17 — Selective Durable Projection Catch-Up

- `P-02`: preserve `catchUpReadSide()` as the simple whole-read-side local
  helper, while adding a Projection-specific public boundary with optional
  target IDs and a historical starting point;
- execute replay as a durable, resumable job with explicit progress and failure
  state rather than one process-local promise;
- coordinate historical and live events through the Inbox so ordering,
  duplicate handling, restart, and multiple-node behavior are explicit;
...
The current helper proves the handler/rebuild core, but it is not equivalent to
the JVM production catch-up process. This Wave closes that capability gap after
package boundaries are stable.

### Wave 18 — Cross-Context Event Exchange
...
- Do not replace or misrepresent the existing local `catchUpReadSide()` helper;
  extend catch-up only through the separately planned selective/durable Wave.
