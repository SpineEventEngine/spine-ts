...
After a nonzero Lerna result, the merge-triggered job may use at most two fresh
same-job retry workspaces. It waits 90, 180, then 360 seconds before strict
registry checks. A complete exact release makes a delayed nonzero result
successful; a partial release selects only exact missing names. The final check
cannot start a fourth Lerna process. Ambiguous registry responses fail closed.
