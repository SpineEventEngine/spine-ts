...
| Delivery server               | port/timeout bounded by options; inbound 4 MiB; record 1 MiB; FIFO 100; batches 1–100; retained defaults 10,000/32 MiB; tracked default 1,000; worker/node IDs 128 bytes | capacity admission is atomic and rejects `RESOURCE_EXHAUSTED`; shutdown rejects new work, completes streams, then loses memory | [README](../../packages/delivery-server/README.md)                                                                                 |
...
| Browser server                | read/write defaults are bounded; unary 1 MiB; auth max 64 active; route body/timeout are finite configured bounds                                                        | timeout covers intake/handler/response; close retries unfinished phases                                                        | [README](../../packages/server/README.md)                                                                                          |
| UnaryGateway/auth             | `maxRequestBytes` is finite positive configuration; binding limits are finite configured operation/timeout bounds                                                        | application owns session policy; expired binding cleanup precedes record deletion                                              | [auth reference](../../packages/auth/REFERENCE.md), [bindings](../../packages/server/src/server/durable-subscription-bindings.ts)  |
