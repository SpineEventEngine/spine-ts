...
| Delivery client               | page 1–1,000; retry/reconnect 0–5; buffer 100; timeout 30,000 ms                                                  | mutations are single-attempt; no retry/backoff is added for removal                                        | [reference](../../packages/delivery-client/REFERENCE.md)                                                                           |
| Delivery server               | inbound 4 MiB; record 1 MiB; batches 1–100; retained 10,000 messages/32 MiB; tracked shards 1,000                 | shutdown rejects new work, completes streams, then loses in-memory state                                   | [README](../../packages/delivery-server/README.md)                                                                                 |
| Client web                    | fetch deadline; 64 updates/1,048,576 bytes/32 lifecycle notices; 5 retries/30,000 ms; 250–5,000 ms jittered delay | reconnect cleanup is local; overflow is terminal rather than dropping state                                | [reference](../../packages/client-web/REFERENCE.md)                                                                                |
...
| Browser server                | auth routes max 64 active; bounded read/write/body/timeout values are configurable positive limits                | timeout covers intake/handler/response; close retries unfinished phases                                    | [README](../../packages/server/README.md)                                                                                          |
| UnaryGateway/auth             | `maxRequestBytes` is a finite configurable positive bound; in-memory binding operation limit is 1                 | application owns session policy; expired binding cleanup precedes record deletion                          | [auth reference](../../packages/auth/REFERENCE.md), [bindings](../../packages/server/src/server/durable-subscription-bindings.ts)  |
