...
const generatedPolicy = Object.freeze({
  ...
  generatedSource(source: string, sources: readonly string[]): string {
    let copiedPreamble = source;
    const leadingBlock = /^\/\*[\s\S]*?\*\/\s*/u.exec(copiedPreamble)?.[0];
    if (leadingBlock !== undefined && /copyright|licen[cs]ed/iu.test(leadingBlock))
      copiedPreamble = copiedPreamble.slice(leadingBlock.length);
    const leadingLines = /^(?:(?:\/\/[^\n]*\n)+)/u.exec(copiedPreamble)?.[0];
    if (leadingLines !== undefined && /copyright|licen[cs]ed/iu.test(leadingLines))
      copiedPreamble = `${leadingLines
        .split("\n")
        .filter((line) => line !== "" && !/copyright|licen[cs]ed/iu.test(line))
        .join("\n")}\n${copiedPreamble.slice(leadingLines.length)}`;
    const withoutCopyright = copiedPreamble.replace(
      /^\/\*\n \* Generated by Spine TypeScript\. Do not edit manually\.\n(?: \* Source Proto: .*\n)+ \*\/\n*/u,
      "",
    );
