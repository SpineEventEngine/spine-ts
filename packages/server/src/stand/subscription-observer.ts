...
function createPredicate(filters: readonly CompositeFilter[], schema: MessageSchema): (state: Message) => boolean {
...
function compositePredicate(filter: CompositeFilter, schema: MessageSchema): (state: Message) => boolean {
  const parts = [
...
function localMask(subscription: Subscription, schema: MessageSchema): readonly string[] | undefined {
  const paths = subscription.topic?.fieldMask?.paths ?? [];
  return paths.length === 0 ? undefined : paths.map((path) => resolvePath(schema, path.split(".")).local.join("."));
}
...
function findField(
  ...
  return (schema?.fields as readonly { readonly name: string; readonly localName: string; readonly message?: MessageSchema }[] | undefined)?.find(
    (field) => field.name === name || field.localName === name,
  );
}
...
function equals(left: unknown, right: unknown, schema?: MessageSchema): boolean {
  ...
  if (isAny(left) && isAny(right)) return sameBytes(toBinary(AnySchema, left), toBinary(AnySchema, right));
...
function packEntityId(state: StandObservedState, id: unknown): Any | undefined {
  ...
  if (idSchema !== undefined && isMessage(id)) return AnyMessages.pack(idSchema, id, { validate: false });
  if (id instanceof Uint8Array) return AnyMessages.pack(BytesValueSchema, create(BytesValueSchema, { value: id }));
  if (typeof id === "string") return AnyMessages.pack(StringValueSchema, create(StringValueSchema, { value: id }));
  if (typeof id === "boolean") return AnyMessages.pack(BoolValueSchema, create(BoolValueSchema, { value: id }));
  if (typeof id === "number") return AnyMessages.pack(DoubleValueSchema, create(DoubleValueSchema, { value: id }));
  return typeof id === "bigint" ? AnyMessages.pack(Int64ValueSchema, create(Int64ValueSchema, { value: id })) : undefined;
}
...
function eventTenant(event: Event): string | undefined {
  switch (event.context?.origin.case) {
    case "importContext": return tenantValue(event.context.origin.value.tenantId);
    case "pastMessage": return tenantValue(event.context.origin.value.actorContext?.tenantId);
...
function tenantValue(tenant: TenantId | undefined): string | undefined {
  switch (tenant?.kind.case) {
    case "value": return tenant.kind.value;
    case "domain": return `domain:${tenant.kind.value.value}`;
    case "email": return `email:${tenant.kind.value.value}`;
    default: return undefined;
  }
}
