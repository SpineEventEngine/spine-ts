...
interface CanonicalMysqlValuesCodec {
...
export const CanonicalMysqlValues: Readonly<CanonicalMysqlValuesCodec> = Object.freeze({
  encode(value: unknown, maxBytes = maxKeyBytes): Uint8Array {
    const encoded = encoder.encode(
      JSON.stringify(CanonicalMysqlValues.encodeValue(value, new WeakSet<object>())),
    );
  ...
  decode(encoded: Uint8Array): unknown {
    try {
      return CanonicalMysqlValues.decodeValue(JSON.parse(decoder.decode(encoded)));
    } catch {
  ...
  encodeValue(value: unknown, ancestors: WeakSet<object>): unknown {
    ...
    if (Array.isArray(value)) {
      ...
      const encoded = [
        "array",
        ...value.map((item) => CanonicalMysqlValues.encodeValue(item, ancestors)),
      ];
      ancestors.delete(value);
      return encoded;
    }
    if (typeof value === "object" && CanonicalMysqlValues.isPlainObject(value)) {
      if (ancestors.has(value))
        throw new Error("MySQL storage identifier has an unsupported value.");
      ancestors.add(value);
      const encoded = [
        ...
        ...Object.keys(value)
          ...
          .map((key) => [
            key,
            CanonicalMysqlValues.encodeValue(Reflect.get(value, key), ancestors),
          ]),
      ];
      ancestors.delete(value);
      return encoded;
  ...
  decodeValue(encoded: unknown): unknown {
    ...
    if (kind === "array") return payload.map(CanonicalMysqlValues.decodeValue);
    if (kind === "object") {
      ...
      for (const entry of payload) {
        ...
        previousKey = entry[0];
        result[entry[0]] = CanonicalMysqlValues.decodeValue(entry[1]);
      }
      return result;
    }
    throw new Error("invalid");
...
export const SortableMysqlColumnValue: Readonly<{
  ...
  encode(value: unknown): { readonly kind: number; readonly data: Uint8Array } {
    if (typeof value === "bigint") {
      ...
      return { kind: 1, data: MysqlIntegers.bytes(value + (1n << 63n)) };
    }
