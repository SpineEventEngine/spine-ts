...
export class OpaqueSessionCookies {
  ...
  constructor(options: OpaqueSessionCookiesOptions) {
    ...
    this.maxHeaderValues = positiveSafeInteger(options.maxHeaderValues ?? 32, "maxHeaderValues");
    this.maxHeaderCharacters = positiveSafeInteger(
      options.maxHeaderCharacters ?? 16_384,
      "maxHeaderCharacters",
    );
    this.maxCookiePairs = positiveSafeInteger(options.maxCookiePairs ?? 64, "maxCookiePairs");
  }

  /** Derives the fixed-length unpadded base64url CSRF value for one session ID. */
  ...
  /** Serializes the immutable pair of host-only cookies for a newly issued session. */
  ...
  /** Serializes the immutable pair of host-only cookie removals. */
  ...
  /** Extracts bearer credentials first, otherwise a fully checked cookie credential. */
  extract(headers: OpaqueSessionHeaders): OpaqueCredentialExtraction {
    if (this.closed) return rejection("closed");
    const values = headerValues(headers, this.maxHeaderValues, this.maxHeaderCharacters);
    if (values === undefined) return rejection("request-too-large");
    const authorization = values.get("authorization") ?? [];
    if (authorization.length > 1) return rejection("duplicate-authorization");
    if (authorization.length === 1) return bearer(only(authorization));
    return this.cookieCredential(values);
  }

  /** Zeroes the owned mutable HMAC secret and rejects all later public operations. */
  close(): Promise<void> {
  ...
  private cookieCredential(
    ...
    const parsed = parseCookies(cookies, this.sessionName, this.csrfName, this.maxCookiePairs);
    if ("reason" in parsed) return rejection(parsed.reason);
    const origin = one(values, "origin", "missing-origin", "duplicate-origin");
    if ("reason" in origin) return rejection(origin.reason);
    if (!this.origins.has(origin.value)) return rejection("forbidden-origin");
    const supplied = one(values, "x-spine-csrf", "missing-csrf", "duplicate-csrf");
    if ("reason" in supplied) return rejection(supplied.reason);
    const expected = this.csrf(parsed.session);
    if (
      !validCsrf(supplied.value) ||
      !sameCsrf(expected, supplied.value) ||
      !sameCsrf(expected, parsed.csrf)
    ) {
      return rejection("csrf-mismatch");
    }
...
function bearer(value: string): OpaqueCredentialExtraction {
  const match = /^bearer ([\x21-\x7e]+)$/i.exec(value);
  return match === null
    ? rejection("malformed-authorization")
    : Object.freeze({ kind: "bearer" as const, value: only(match.slice(1)) });
}
...
function parseCookies(
  ...
  for (const header of headers) {
    if (hasForbiddenCookieCharacter(header)) return { reason: "malformed-cookie" };
    for (const pair of header.split(";")) {
      ...
      if (!validCookiePair(name, value)) return { reason: "malformed-cookie" };
  ...
  if (
    session.length !== 1 ||
    csrf.length !== 1 ||
    !validSessionId(only(session)) ||
    !validCsrf(only(csrf))
  ) {
    return { reason: "malformed-cookie" };
  }
  return { session: only(session), csrf: only(csrf) };
}

function one<T extends OpaqueCredentialRejection["reason"]>(
  values: ReadonlyMap<string, readonly string[]>,
  name: string,
  missing: T,
  duplicate: T,
  ...
  return { value: only(entries) };
}

function only(values: readonly string[]): string {
  const value = values[0];
  if (value === undefined) throw new Error("expected one header value");
  return value;
}

function hasForbiddenCookieCharacter(value: string): boolean {
...
function validCookieName(value: string, name: string): string {
...
function validCookiePair(name: string, value: string): boolean {
...
function validCsrf(value: string): boolean {
  return value.length === 43 && base64url(value);
}
...
function sameCsrf(expected: string, supplied: string): boolean {
  if (expected.length !== supplied.length) return false;
  return timingSafeEqual(Buffer.from(expected), Buffer.from(supplied));
}

function rejection(reason: OpaqueCredentialRejection["reason"]): OpaqueCredentialRejection {
