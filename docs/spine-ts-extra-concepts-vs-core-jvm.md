...
| delivery inbox, shards, pickup, stations                          | JVM `server/delivery/Inbox.java`, `ShardedWorkRegistry.java`, `Station.java`, and `Conveyor.java`                                                                                                   | TS delivery concepts are not invented, although its lifecycle coordinator may be. JVM Projection catch-up is not implemented in TS; the legacy-named local reset/replay helper is not an equivalent. |
...
- **Verified parity:** default command routing uses the first declared field.
  Current JVM `DefaultCommandRoute` delegates to `ByFirstField`, which reads
  descriptor field index zero and rejects repeated or map fields. TS retains
  that declaration-order policy with additional fail-closed value guards.
