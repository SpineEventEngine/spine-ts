...
release-scope decision. Only the first category is a candidate for
streamlining. The comparison target is the fresh `master` tree of
...
| `InMemoryRecordStorage`, tenant slices, `StorageContext`                                                               | `packages/storage/src/memory`                          | Test/default adapter adaptation                               | Keep as a reference adapter; avoid treating tenant-slice implementation as domain API.                        |
...
| `ServerEnvironment` and environment attachment/generation lifecycle                                                    | `packages/server/src/server`                           | Node lifecycle adaptation with unusually explicit state       | Candidate for internal consolidation; preserve observable start/stop/close ordering.                          |
...
| explicit `TypeRegistry`, `deriveTypeUrl`, `packAny` facade                                                             | `packages/core/src/index.ts`                           | TS wire-contract facade                                       | Consolidate helper names if all callers use one registry/packing boundary.                                    |
...
| durable subscription JSON/Base64 records, claim/cancel markers, CAS ownership                                          | `packages/server/src/services/subscription-records.ts` | TS persistence compatibility mechanism                        | Replace JSON records with a typed protobuf record once wire compatibility permits.                            |
| `BoundedContextFixture` and testing-only assembly                                                                      | `packages/testing`                                     | Test ergonomics adaptation                                    | Keep outside runtime packages; avoid duplicating production context builders.                                 |
