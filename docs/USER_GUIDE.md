...
The packages in this source repository are currently private at version `0.0.0`.
They are not published to npm. The Wave 3 tarball tests prove the intended
external-consumer shape by packing and extracting local artifacts; npm
publication is deliberately deferred until after all waves. While developing in
this repository, install workspace dependencies and generate before building:
...
users-model/
  ...
  proto/acme/users/v1/users.proto
...
The Chat model declares Users as both an npm dependency and a
...
copy of `users_pb.ts` inside Chat.

```json
{
  "formatVersion": 1,
  "mode": "model",
  "packageName": "@acme/chat-model",
  "protoRoot": "proto",
  "generatedRoot": "generated",
  "exportRoot": "generated",
  "dependencies": ["@spine-event-engine/proto", "@acme/users-model"],
  "moduleExport": "chatProtoModule"
}
```
...
import { ChatSchema } from "@acme/chat-model/generated/acme/chat/v1/chat_pb.js";
