...
const server = await startDatastoreOrdersServer(new InMemoryStorageFactory(), { host: "127.0.0.1", port: 0 });
try {
  console.log(JSON.stringify(await runDatastoreOrdersLoad({ baseUrl: server.baseUrl, users: requestedUsers as 10 | 100 | 1000 })));
} finally {
  await server.close();
}
