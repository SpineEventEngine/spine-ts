...
describe("MessageBoard app manifest", () => {
  it("keeps exact runtime versions and starts already-built local output", async () => {
    ...
    expect(manifest.dependencies["@spine-event-engine/core"]).toBe("2.0.0-snapshot.10");
    expect(manifest.dependencies["@spine-event-engine/proto"]).toBe("2.0.0-snapshot.10");
    expect(manifest.dependencies["@spine-event-engine/server"]).toBe("2.0.0-snapshot.10");
    expect(manifest.devDependencies?.["@spine-event-engine/proto-tools"]).toBe("2.0.0-snapshot.10");
