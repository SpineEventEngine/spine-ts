...
const root = fileURLToPath(new URL("../packages/client-react", import.meta.url));
const forbidden = [
  /(?:from\s+|import\s*\()?['"]node:/u,
  /@connectrpc\/connect-node/u,
  /(?:from\s+|import\s*\()?['"](?:fs|path|process)(?:['"/])/u,
  /(?:from\s+|import\s*\()?['"](?:react-dom\/server|react-dom\/client)(?:['"/])/u,
];
...
for (const path of await files(root)) {
  ...
  for (const pattern of forbidden) {
    if (pattern.test(source))
      throw new Error(`client-react forbidden dependency ${String(pattern)} in ${path}`);
  }
}
