...
describe("release policy", () => {
  ...
  it("fails closed for each repository policy boundary", () => {
    ...
    const duplicatePublicName = globalThis.structuredClone(entries);
    duplicatePublicName[2].manifest.name = duplicatePublicName[1].manifest.name;
    expect(() => validateReleasePolicy(duplicatePublicName)).toThrow("public-name inventory");
    const privatePublicPackage = globalThis.structuredClone(entries);
    privatePublicPackage[1].manifest.private = true;
    expect(() => validateReleasePolicy(privatePublicPackage)).toThrow("must not be private");
    const publicExample = globalThis.structuredClone(entries);
    publicExample.at(-1).manifest.private = false;
    expect(() => validateReleasePolicy(publicExample)).toThrow("must be private");
    ...
    expect(() => validateReleasePolicy(wrongPin)).toThrow("must use 2.0.0-snapshot.10");
  });
});
